...
function loadStatsPage()
{
	...
	{
		CUScore = table.rows[rcounter].cells[2].innerHTML;
		CUScore = parseInt(CUScore);
		oScore = table.rows[rcounter].cells[3].innerHTML;
		oScore = parseInt(oScore);

		if(oScore > CUScore)
		{
			winner = table.rows[rcounter].cells[1].innerHTML;
			losses++;
		}
		else
		{
		...
		table.rows[rcounter].cells[4].innerHTML = winner;
	}
...
function loadPlayersPage()
{
	...
	for(count = 0; count < players.length; count++)
	{
		...
		anc.setAttribute('class', "dropdown-item");
		anc.setAttribute('href', "#");
		if(count == 0)
		{
			anc.setAttribute('onclick', "switchPlayers(0)");
		}
		if(count == 1)
		{
			anc.setAttribute('onclick', "switchPlayers(1)");
		}
		if(count == 2)
		{
			anc.setAttribute('onclick', "switchPlayers(2)");
		}
		if(count == 3)
		{
			anc.setAttribute('onclick', "switchPlayers(3)");
		}
		anc.innerHTML = players[count].name;
		myDiv.appendChild(anc);
	}
	...
	// anc.setAttribute('href', "#");
	// anc.setAttribute('onclick', "switchPlayers(3)");
	// anc.innerHTML = players[3].name;
	// myDiv.appendChild(anc);

	// var count;
	// for(count = 0; count < players.length; count++)
	// {
	// 	var idee = document.getElementById("player_selector");
	// 	document.write('a');
	// 	document.
	// }
}
